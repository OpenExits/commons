...
| File | Shape |
|---|---|
| `sites.geojson` | One Point Feature per **site** (centroid), properties: id, path, name, country, region, city, status, access, sensitivity, exit/landing counts, updatedAt |
| `features.geojson` | One Point Feature per **feature** (exit, landing, parking…): role, per-feature fields, flattened key measurements with their `measuredAt` |
| `sites.csv` | Flat core scalars, one row per site |
...
- **Sensitivity (rule OE-R08):** the public Commons only contains `public` sites, but if
...
`updatedAt` per site, or diff the git history between two tags for a precise changelog.
